...
| 规则 | 实现 |
|---|---|
| 新增 id | `max(已有 id) + 1`,删除后不复用 |
| 时间戳 | 带本地时区偏移(`+08:00`),**不写 `Z`** |
| 写盘 | 读-改-写 + 原子替换;写前比对 `mtimeMs` 与文件长度,被改过则重读重算一次 |
...
|---|---|---|
| `date` | `day` | 都是本地日期 |
| `start` / `end` | `time` | 共享格式只有开始时间;PHL 侧补默认 1 小时作为 `end` |
| `title` | `title` | |
| `notes` | `note` | |
| `sharedScheduleId` | `id` | PHL 侧新增字段,用来认出"这条来自共享日程" |
| `repeatWeekdays` | —— | 每周重复的日程**不写入**共享文件(格式不支持),只在 PHL 内保存 |

同步方向与边界(重要):

- **读入**:启动后把共享文件里还不存在的条目补进 PHL 日历(只新增,不改不删)。
- **写出**:PHL 新增/修改可表示的日历条目时写回共享文件;删除时,只有
  "内容仍与 PHL 记录一致"的条目才会从共享文件里删除——对方改过的条目会保留。
- **PHL 不会因为共享文件里少了一条就删除本地日程**;本地数据只由用户操作删除。
- 每周重复、无日期、标题为空的条目不参与共享。
