...
这是整合预览，不代表 Hello Pinghe! Launcher 的全部功能已经移植。心履服务器端 AI 暂按其原有接口运行，尚未读取 PH Launcher 数据；若要共享课程、成绩或邮件，仍需另行设计并部署双方明确同意的数据接口。AI 代发邮件／代交作业、邮件附件发送、自动标记已读、Word 起草、任意班级和全校空教室查询、不经确认的写入仍未在此版实现。学校数据不能替代官方记录。详见 [1.0 更新说明](docs/PH-Launcher-1.0-更新说明.md)。
