...
function harness(change = () => {}, scope = 'mail') {
  ...
  const box = {
    ...
    shouldOfferLauncherTools: () => true, shouldOfferMailTools: () => true,
    ...
    requestAiTurn: async (_config, messages) => {
  ...
  return { state, run: () => box.chat([{ role: 'user', content: 'Read my email and tasks' }]) };
}
