...
test('transient loading and error states translate while mounted and round trip', async () => {
  const {document,i18n}=harness('<section id="status"></section>');
  const target=document.querySelector('#status');
  const phrases=['正在打开你的词本…','正在检查到期词…','正在查询本机词典…','正在准备离线词库…','正在打开邮件…','正在读取本地记录…','词本暂时没能打开','已刷新，目前没有到期词'];
  i18n.apply('en');
  for (const phrase of phrases) {
    target.textContent=phrase; await tick();
    assert.notEqual(target.textContent,phrase,phrase);
    i18n.apply('zh-CN'); assert.equal(target.textContent,phrase);
    i18n.apply('en'); assert.notEqual(target.textContent,phrase);
  }
});
...
test('textarea placeholders and option-group labels translate without changing any draft', async () => {
  const { document, window, i18n } = harness('<textarea placeholder="密码">保存</textarea><select><optgroup label="设置"><option value="unchanged">保存</option></optgroup></select><div class="vocab-reader-text">关闭</div>');
  let eventLanguage;
  window.addEventListener('ph:language-changed', event => { eventLanguage = event.detail.language; });
  i18n.apply('en');
  assert.equal(document.querySelector('textarea').placeholder, 'Password');
  assert.equal(document.querySelector('textarea').value, '保存');
  assert.equal(document.querySelector('optgroup').getAttribute('label'), 'Settings');
  assert.equal(document.querySelector('option').value, 'unchanged');
  assert.equal(document.querySelector('.vocab-reader-text').textContent, '关闭');
  assert.equal(eventLanguage, 'en');
  i18n.apply('zh-CN');
  assert.equal(document.querySelector('textarea').placeholder, '密码');
});
