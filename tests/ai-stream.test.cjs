...
test('streaming remains text-only while launcher tools use the confirmed tool path', () => {
  ...
  assert.match(source, /const launcherTools = controlEnabled && shouldOfferLauncherTools\(working\) \? AI_TOOLS : \[\]/);
...
test('only an enabled configured local model is prewarmed without blocking startup', () => {
  ...
  assert.match(source, /cancelLocalAiWarmup\(\);\s*vocabularyStudy\?\.invalidate\(\);\s*aiLauncherReader = null;\s*const saved = secureStore\.updateAi\(config \|\| \{\}\);\s*scheduleLocalAiWarmup\(250\);/);
});
