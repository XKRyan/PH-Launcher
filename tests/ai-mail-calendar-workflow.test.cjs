...
function workflowHarness() {
  ...
  return {
    ...
    run: (content = '查看我邮箱中关于大学宣讲会的内容并写入日程') => box.runAiChat([{ role: 'user', content }]),
...
test('real tool gates keep ordinary conversation free of launcher and mail data tools', async () => {
  const app = workflowHarness();
  const result = await app.run('你好');
  assert.equal(app.offered[0].length, 0);
  assert.deepEqual(app.mailCalls, []);
