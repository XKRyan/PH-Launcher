(() => {
  ...
  function timetable() {
    ...
    const toolbar = `<div class="school-weekbar"><strong>${esc(start)} — ${esc(shift(start, state.showWeekend ? 6 : 4))}</strong><div class="school-actions">${btn('上一周', 'week', 'data-delta="-7" aria-label="上一周"')}${btn('本周', 'week', 'data-delta="0"')}${btn('下一周', 'week', 'data-delta="7" aria-label="下一周"')}${btn(state.showWeekend ? '隐藏周末' : '显示周末', 'toggle-weekend', `aria-pressed="${state.showWeekend}"`)}${btn(state.busy.has('edupage') ? '正在同步…' : '刷新课表', 'sync', `data-source="edupage" ${state.busy.has('edupage') ? 'disabled' : ''}`, true)}</div></div>${data && !classRoute ? `<div class="school-actions">${btn('选择教学组', 'groups')}${btn(courseReminderLabel(), 'course-reminder', `${selections()?.length && selectedLessons().some((lesson) => !lesson.cancelled) ? '' : 'disabled'}`)}${btn('加入计划课表', 'import-plan', `${selections()?.length && selectedLessons().some((lesson) => !lesson.cancelled) ? '' : 'disabled'}`)}</div>` : ''}`;
  ...
  function groupsDialog() {
    ...
    const inferred = automatic ? inference.keys : [];
    const inferenceNote = automatic
