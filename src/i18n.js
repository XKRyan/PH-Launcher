(() => {
  ...
  const protectedContent = [
    ...
    '.vocab-meaning', '.vocab-question', '.vocab-own-example p', '.vocab-reading-text', '.vocab-reader-text', '.vocab-context-hint',
  ...
  function translateElement(element) {
    // Field values and user text are protected, but their UI placeholder is not.
    const field = ['INPUT', 'TEXTAREA'].includes(element.tagName);
    if (blocked(element) && !(field && !element.closest('[data-i18n-ignore], [translate="no"]'))) return;
  ...
  function apply(value) {
    ...
    window.dispatchEvent(new window.CustomEvent('ph:language-changed', { detail: { language } }));
  }
  ...
  window.i18n = { t, apply, mount, settings, isProtected: blocked, locale: () => language };
})();
