'use strict';
const { occursOn } = require('./calendar.cjs');
...
function validReminder(input) {
  ...
  const result = { id, title, body: String(input.body || '').trim().slice(0, 800), dueAt };
  if (input.calendarEventId !== undefined || input.occurrenceDate !== undefined) {
    if (!/^[a-z0-9-]{1,80}$/i.test(String(input.calendarEventId || '')) || !/^\d{4}-\d{2}-\d{2}$/.test(String(input.occurrenceDate || ''))) throw new Error('提醒格式无效');
    result.calendarEventId = String(input.calendarEventId);
    result.occurrenceDate = String(input.occurrenceDate);
  }
  return result;
}

function dateKey(date) { return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`; }
function calendarReminderItems(events, now = Date.now()) {
  const items = [];
  ...
  for (const event of Array.isArray(events) ? events : []) {
    ...
    const dates = event.repeatWeekdays?.length ? Array.from({ length: 8 }, (_, offset) => { const date = new Date(today); date.setDate(date.getDate() + offset); return date; }).filter(date => occursOn(event, date)).map(dateKey) : [event.date];
    for (const occurrenceDate of dates) {
      const state = event.reminderState?.[occurrenceDate];
      if (state?.action === 'completed' || state?.action === 'cancelled') continue;
      const normalDueAt = new Date(`${occurrenceDate}T${event.start}:00`).getTime() - minutes * 60_000;
      const dueAt = state?.action === 'snoozed' ? state.snoozedUntil : normalDueAt;
      const id = `calendar:${event.id}:${occurrenceDate}:${event.start}:${minutes}`;
      items.push({ id, title: `日程提醒：${event.title}`, body: `${minutes === 0 ? '现在开始' : `${minutes} 分钟后`} · ${occurrenceDate} ${event.start}${event.notes ? `\n${String(event.notes).slice(0, 400)}` : ''}`, dueAt, calendarEventId: event.id, occurrenceDate });
    }
  }
  return items;
}
...
class ReminderScheduler {
  ...
  syncCalendar(events) {
    this.syncGroup('calendar:', calendarReminderItems(events, this.now()));
  }
  arm() {
...
module.exports = { ReminderScheduler, validReminder, calendarReminderItems, MAX_PENDING_REMINDERS };
