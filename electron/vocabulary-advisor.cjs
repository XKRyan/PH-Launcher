...
function systemPrompt() {
  return [
    ...
    'Use no more than the requested limit. Each sentence must contain its target word or phrase exactly once.',
  ].join(' ');
}
...
function validateModelResult(raw, candidates, limit) {
  let result;
  try { result = JSON.parse(raw.trim()); } catch { throw new Error('AI 返回的词汇建议格式无效'); }
...
function createVocabularyAdvisor({ getConfig, fetchImpl = globalThis.fetch, timeoutMs = DEFAULT_TIMEOUT_MS } = {}) {
  ...
  return async function adviseVocabulary(input, { signal } = {}) {
    ...
    try {
      const raw = await Promise.race([
        Promise.resolve().then(() => requestModel({ selected, messages: requestMessages(data), signal: controller.signal, fetchImpl })),
        stopped,
      ]);
      if (signal?.aborted) throw abortError(signal.reason);
